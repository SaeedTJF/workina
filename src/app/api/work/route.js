...
export async function POST() {
  ...
  const wp = await prisma.workPeriod.create({ data: { userId: user.id, startedAt: new Date() } })
...
export async function GET() {
  ...
  if (!user) return NextResponse.json({ message: 'Unauthorized' }, { status: 401 })
  await ensureMidnightStops(user.id)
  const periods = await prisma.workPeriod.findMany({ where: { userId: user.id }, orderBy: { createdAt: 'desc' } })
...
export async function PUT(request) {
  ...
  if (!user.isAdmin && period.userId !== user.id) return NextResponse.json({ message: 'Forbidden' }, { status: 403 })
